...
while (r-1>i) :
    ...
    print(dt1)
    """
    dt1='("'+xldf.iloc[i]['Cell']+'","'+xldf.iloc[i]['Site']+'","'+xldf.iloc[i]['2G/3G']+'","'+\
	xldf.iloc[i]['Division']+'","'+xldf.iloc[i]['District']+'","'+xldf.iloc[i]['Thana']+'","'+\
	xldf.iloc[i]['LAC']+'","'+xldf.iloc[i]['CI']+'","'+\
    xldf.iloc[i]['Antenna DIRECTION']+'","'+xldf.iloc[i]['LATITUDE']+'","'+xldf.iloc[i]['LONGITUDE']+'","'+xldf.iloc[i]['Site ADDRESS']+'","'+\
    xldf.iloc[i]['location']+'")'
    ...
    if i%10000==0:
        ...
        query = "insert into lookups.lkp_btsaddress (cell_name,enodeb_name_site,network_type,division,district,thana,cell_id,tac_lac,antenna_direction,latitude,longitude,`address`,`location`) PARTITION(eventdate='"+dateval+"') values "+insertStr
        cur.execute(query)
        insertLs.clear()
